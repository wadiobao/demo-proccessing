...
public class FormService implements IFormService{
	...
	@Override
	public StateResponse<Object> getAllForm() {
		List<Form> all = formRepository.findAllByOrderByNgayDangDesc();
		List<FormResponse> responses = new ArrayList<FormResponse>();
		for (Form form : all) {
			
			FormResponse response = FormResponse.builder().formId(form.getFormId()).tacGia(form.getTacGia()).tieuDe(form.getTieuDe())
					.tags(form.getTags()).ngayDang(form.getNgayDang()).noiDung(form.getContent().getNoiDung()).topic(form.getTopic().getTopic()).build();
	...
	public StateResponse<Object> newForm(Long topicId,FormRequest formRequest) {
		...
		Form form = Form.builder().tacGia(name).tieuDe(formRequest.getTieuDe()).tags(tags).ngayDang(new Date()).topic(topic).build();
		...
		return StateResponse
				.builder().result(FormResponse.builder().formId(form.getFormId()).tacGia(form.getTacGia()).tieuDe(form.getTieuDe())
						.tags(form.getTags()).ngayDang(form.getNgayDang()).noiDung(content.getNoiDung()).topic(topic.getTopic()).build())
				.build();
	...
	@Override
	@Transactional
	public StateResponse<Object> getFormComment(String formId){
		List<Comment> comments = commentRepository.findByForm_FormId(formId);
		List<CommentResponse> responses = new ArrayList<CommentResponse>();
		for (Comment comment : comments) {
			CommentResponse  response = CommentResponse.builder().id(comment.getCommenttId()).tacGia(comment.getUser().getUserName()).noiDung(comment.getNoiDung()).ngayComment(comment.getNgayComment()).build();
			responses.add(response);
		}
		return StateResponse.builder().result(responses).build();
	...
	@Override
	public StateResponse<Object> getAllTopic(){
		...
		for (Topic topic : topics) {
			...
			for (Form form : forms) {
				FormResponse formResponse = FormResponse.builder().formId(form.getFormId()).tacGia(form.getTacGia()).tieuDe(form.getTieuDe())
						.tags(form.getTags()).ngayDang(form.getNgayDang()).noiDung(form.getContent().getNoiDung()).topic(topic.getTopic()).build();
				formResponses.add(formResponse);
			}
			TopicResponse response = TopicResponse.builder().topicId(topic.getTopicId()).topic(topic.getTopic()).forms(formResponses).build();
			responses.add(response);
		}
		return StateResponse.builder().result(responses).build();
	}
	...
	@Override
	public StateResponse<Object> getAllTopics(){
		List<Topic> topics = topicRepository.findAll();
		List<TopicResponse> responses =  new ArrayList<TopicResponse>();
		for (Topic topic : topics) {
			TopicResponse response = TopicResponse.builder().topicId(topic.getTopicId()).topic(topic.getTopic()).build();
			responses.add(response);
		}
		return StateResponse.builder().result(responses).build();
	}
	...
	@Override
	public StateResponse<Object> getAllFormFromTopic(Long topicId) {
		List<Form> all = formRepository.findByTopic_TopicIdOrderByNgayDangDesc(topicId);
		List<FormResponse> responses = new ArrayList<FormResponse>();
		for (Form form : all) {
