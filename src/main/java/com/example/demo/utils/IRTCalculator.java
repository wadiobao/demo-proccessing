...
public class IRTCalculator {
	public static double p(double theta, double b) {
	    return 1.0 / (1.0 + Math.exp(-(theta - b)));
	}

	public static double likelihood(int x, double theta, double b) {
	    double p = p(theta, b);
	    return x == 1 ? p : (1 - p);
	}

	
	public static double logLikelihood(int x, double theta, double b) {
	    double p = p(theta, b);
	    return x * Math.log(p) + (1 - x) * Math.log(1 - p);
	}

	public static double totalLogLikelihood(List<UserAnswer> answers, double theta) {
	    double sum = 0;
     ...
     */
    public static double gradientMAP(List<UserAnswer> answers, double theta, double sigma) {
     ...
     */
    public static double hessianMAP(List<UserAnswer> answers, double theta, double sigma) {
     ...
     */
    public static double estimateThetaMAP(
	...
	public static double fisher(double theta, double b) {
	    double p = p(theta, b);
	    return p * (1 - p);
	}
	
	public static double suggestDifficultyB(double thetaCurrent, double pTarget) {
        // b = theta + ln((1-P)/P)
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < numQuestionsToSimulate; i++) {
		    ...
		    // Dùng P(thetaCurrent, actualB) để xác định xác suất, sau đó dùng Random để chọn True/False
		    double probabilityOfCorrect = p(thetaCurrent, actualB);
		    boolean isCorrect = new Random().nextDouble() < probabilityOfCorrect;
		    ...
		    System.out.println(fisher(thetaCurrent, actualB));
